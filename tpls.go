...
<script type="text/ng-template" id="/inline-edit.html">
    <td ng-mouseenter="roll=true" ng-mouseleave="roll=false" class="tdrename"
        ...
        <a  ng-click="Go(item.Name)" ng-if="item.IsDir" class="dir">{{ item.Name }}</a><span class="show-xs small"><br />{{ item.ModTime|date:'dd/MM/yyyy HH:mm:ss' }} | {{ item.Size|bytes }}</span>
